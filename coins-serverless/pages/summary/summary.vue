...
export default {
	...
	methods: {
		...
		updateData() {
			...
			this.reqSummary('updateSummary', formdata).then(res => {
				console.log(res);
			});
		},
		getData() {
			...
			this.reqSummary('getSummary', formdata)
				.then(res => {
					if (res.code == 0 && res.list.length !== 0) {
						this._id = res.list[0]._id;
						this.target = res.list[0].target || '';
						this.book = res.list[0].book || '';
						this.movie = res.list[0].movie || '';
						this.significant = res.list[0].significant || '';
						this.goodSentence = res.list[0].goodSentence || '';
						this.stupid = res.list[0].stupid || '';
						this.point = res.list[0].point || '';
					} else {
						this.getData();
					}
				})
