		...
		<view class="content">
			...
			<view class="addEvent">
				...
				<view class="item arrow" @click="selectTag">
					<text>标签</text>
					<view v-if="tag.length == 0" style="color:#666666;">请选择类型</view>
				...
				<view class="bottom-btn flex-center" @click="updateTime"><view class="btn">完成</view></view>
			</view>
			...
			<uni-drawer :visible="true" mode="right" @close="selectTag" v-if="showSelectTag">
				<view class="selectTag">
					...
					<view class="item" @click="confirmTag" v-for="(item, index) in tagList" :key="index" :data-index="index">
...
export default {
	...
	methods: {
		...
		updateTime() {
			...
			this.reqTime('updateTime', formdata).then(res => {
				uni.navigateBack({
					delta: 1
				});
			});
