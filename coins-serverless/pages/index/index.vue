<template>
	<view>
		...
		<view class="content">
			...
			<!-- 时间列表 -->
			<view class="table" v-for="(item, index) in tableList" :key='index'>
				<view class="left-table">{{ item.time }}</view>
				<view class="right-table" :class="statusText[item.tag][0]" @click="toAddEve" :data-index="index">{{ item.event }}</view>
			</view>
...
export default {
	...
	methods: {
		...
		getTimeData() {
			...
			this.reqTime('getTime', formdata).then(res => {
				if (res && res.code === 0) {
