<template>
	<view class="home">
		...
		<!-- 日历 -->
		<view>
			<u-calendar v-model="showDay" :mode="mode" @change="change"></u-calendar>
		</view>
		...
		<swiper :style="{height:swiperHeight}" class="swiper-group" :current="swiperCurrent" @transition="transition"
				...
				<scroll-view scroll-y="true" @scrolltolower="onreachBottom" class="scrol">
					...
					<view class="">
						...
						<block v-if="todos.length!=0">
							<view class="todos">
								<view class="item" :class="item.completed ? ' completed' : ''" v-for="(item,index) in todos" :key="index"
								 @click="toggleTodoHandle" :data-index="index" :data-_id="item._id" :data-completed="item.completed">
							...
							<view class="footer">
								<text class="btn" @click="toggleAllHandle">选择全部</text>
								<text class="btn" v-if="todos.length > leftCount" @click="clearCompletedHandle">清除完成项</text>
							</view>
						</block>
						<block v-else>
							<view class="empty">
								<text class="title">没有任务，放松一下！</text>
							</view>
						</block>
					</view>
				</scroll-view>
					...
					<view class="timer" :class="isRuning ? 'timer--runing': ''">
							...
							<view class="settingBox">
								<view class="section panel">
									...
									<view class="section_body">
										<slider @change="changeWorkTime" show-value="true" min="1" max="60" :value="workTime" left-icon="cancel"
								...
								<view class="section panel">
									...
									<view class="section_body">
										<slider @change="changeRestTime" show-value="true" min="5" max="60" :value="restTime" left-icon="cancel"
										 right-icon="success_no_circle" />
									</view>
								</view>
							</view>
						...
						<view class="timer_main">
							...
							<view class="timer_time-wrap">
								...
								<!-- <text v-if="isRuning" :animation="nameAnimation" class="timer_taskName">{{taskName}}{{completed ? '已完成！' : '中'}}</text> -->
								<image v-if="completed" class="timer_done" src="../../static/image/complete.png"></image>
							</view>
						</view>

						<view class="timer_footer">
							<view @click="startTimer" data-type="work" class="timer_ctrl" :class="isRuning && timerType == 'rest' ? 'hide' : ''">{{isRuning ? '完成': '工作'}}</view>

							<view @click="startTimer" data-type="rest" class="timer_ctrl" :class="isRuning && timerType == 'work' ? 'hide' : ''">{{isRuning ? '完成': '休息'}}</view>
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//获取Todo列表
			getTodoList: function(e) {
				let formdata = {
					userID: this.userID,
					date: this.fulldate
				}
				this.$u.post('/api/todo/getInfo', formdata).then(res => {
					console.log('获取Todo列表===>', res);
					if (res.code == 2000) {
						this.input = ''
						this.todos = res.list
					}
			...
			addTodoHandle: function(e) {
				...
				this.$u.post('/api/todo/add', formdata).then(res => {
					console.log(res);
					if (res.code == 2000) {
						this.input = '',
			...
			toggleTodoHandle: function(e) {
				...
				this.$u.post('/api/todo/update', formdata).then(res => {
					if (res.code == 2000) {
						var todos = this.todos
			...
			removeTodoHandle: function(e) {
				...
				this.$u.post('/api/todo/del', formdata).then(res => {
					if (res.code == 2000) {
						var todos = this.todos
			...
			stopTimer: function() {
				...
				this.leftDeg = initDeg.left,
					this.rightDeg = initDeg.right
			...
			changeRestTime: function(e) {
				...
				this.restTime = restTime,
					this.remainTimeText = restTime + ':00'
			}
		}
	}
</script>
